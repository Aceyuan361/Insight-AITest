...
from insight_eyes.public.android.device_profile import (
    get_device_profile,
    DeviceProfile,
    CollectionStrategy,
    Vendor,
...
class DeviceDetector:
    ...
    def detect(self):
        ...
        if self._vendor_info is None:
            ...
            logger.info(f"设备检测完成: vendor={self._vendor_info.get('vendor', 'unknown')}, "
...
class SurfaceStatsCollector(object):
    ...
    """
    def __init__(self, device, frequency, package_name, fps_queue, jank_threshold, surfaceview, use_legacy=False, device_profile=None):
        self.device = device
        ...
        # ============ 设备配置系统 ============
        # 获取或创建设备配置档案
        self.device_profile: DeviceProfile = device_profile if device_profile else get_device_profile(device)
        self.strategy: CollectionStrategy = self.device_profile.get_strategy()

        # 记录设备配置信息
        logger.info(f"[设备配置] 厂商={self.device_profile.vendor.value}, "
                   f"ROM={self.device_profile.rom_type.value}, "
                   f"Android={self.device_profile.android_version}, "
                   f"型号={self.device_profile.model}")
        logger.info(f"[FPS策略] 主方法={self.strategy.fps_primary_method}, "
    ...
    def get_latest_fps_data(self):
        ...
        if acquired:
            try:
                ...
                logger.debug(f"[get_latest_fps_data] 成功获取锁: fps={current_fps}, jank={self.collect_jank}")
                # 直接返回实例变量中的最新数据
                return {
        ...
        while True:
            try:
                ...
                if self.use_legacy_method:
                    ...
                    if fps > 60:
                        fps = 60
                    logger.debug(f"[FPS计算线程] legacy模式: frame_count={frame_count}, seconds={seconds:.2f}, fps={fps}")
                    ...
                    logger.debug(f"[FPS计算线程] gfxinfo模式: frames={len(timestamps)}, fps={fps}, jank={jank}")
                    # 线程安全地更新实例变量，添加超时机制
                    acquired = self._fps_data_lock.acquire(timeout=2.0)
                    if acquired:
                        try:
            ...
            try:
                ...
                if self.use_legacy_method:
                    ...
                    if refresh_period is None or new_timestamps is None:
                        ...
                        self.focus_window = self.get_focus_activity()
                        logger.debug("[FPS采集线程] refresh_period is None or timestamps is None，跳过本次采集")
                        continue
                    # 计算不重复的帧
                    ...
                    try:
                        ...
                        logger.debug(f"[FPS采集线程] 成功放入队列: frames={len(timestamps)}, refresh_period={refresh_period}")
                    except queue.Full:
                        # 队列已满，移除最旧的元素
                        try:
                            self.data_queue.get_nowait()
        ...
        try:
            ...
            if gfx_results and gfx_results.strip():
                ...
                if len(gfx_results) > 0:
                    ...
                    if timestamps and len(timestamps) >= 2:
                        refresh_period = (timestamps[-1][1] - timestamps[0][1]) / len(timestamps)
                        logger.info(f"[策略2.5成功] gfxinfo framestats 宽松解析: {len(timestamps)} 帧")
                        return (refresh_period, timestamps)
                    else:
    ...
    def _parse_gfxinfo_summary(self, summary_output: str) -> list:
        ...
        try:
            ...
            # 如果有总帧数，生成时间戳
            if total_frames > 0:
                logger.debug(f"gfxinfo summary: {total_frames} 总帧, {janky_frames} 卡顿帧 ({janky_percent}%)")
                logger.debug(f"百分位帧时间: 50th={frame_time_50th}ms, 90th={frame_time_90th}ms, "
                ...
                for i in range(sample_frames):
                    ...
                    if rand_val < janky_ratio * 100:
                        ...
                        if frame_time_90th > 0:
                            frame_time_ms = frame_time_90th + (rand_val / 100.0) * (frame_time_95th - frame_time_90th)
                        else:
                        ...
                        frame_time_ms = frame_time_90th if frame_time_90th > 0 else base_frame_time_ms * 1.2
                    else:
                        # 慢帧（95%-100%）：使用 95th 百分位
                        frame_time_ms = frame_time_95th if frame_time_95th > 0 else base_frame_time_ms * 1.5
...
class FPSMonitor(object):
    ...
    def __init__(self, device_id, package_name=None, frequency=1.0, timeout=24 * 60 * 60, fps_queue=None,
                 jank_threshold=166, use_legacy=False, surfaceview=True, start_time=None,
                 device_profile=None, **kwargs):
        """
        构造器
        ...
        if not package_name:
            logger.warning(f"FPSMonitor初始化时package_name为空，将使用空字符串")
            package_name = ""
        ...
        try:
            if hasattr(self, 'fpscollector') and self.fpscollector:
                ...
                has_collector_thread = (
                    hasattr(self.fpscollector, 'collector_thread') and
                    self.fpscollector.collector_thread and
                    self.fpscollector.collector_thread.is_alive()
                )
                has_calculator_thread = (
                    hasattr(self.fpscollector, 'calculator_thread') and
                    self.fpscollector.calculator_thread and
                    self.fpscollector.calculator_thread.is_alive()
                )
