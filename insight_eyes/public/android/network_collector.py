...
class NetworkCollector:
    ...
    def __init__(self, device_id: str, device_profile=None):
        ...
        if self.strategy:
            logger.debug(f"[网络采集器] 使用策略: {self.strategy.network_method}, "
    ...
    def _get_app_uid(self, package_name: str) -> Optional[int]:
        ...
        try:
            ...
            result = adb.shell(f'ls -l /data/data/', self.device_id)
            if result:
                for line in result.split('\n'):
    ...
    def _calculate_flow_rate(self, package_name: str, current_rx: int, current_tx: int) -> Dict[str, float]:
        """
        计算网络流量速率
        ...
        if time_delta <= MIN_TIME_INTERVAL:
            if time_delta <= 0:
                ...
                logger.debug(f"[网络采集] 时间间隔过小: {time_delta}s < {MIN_TIME_INTERVAL}s，跳过本次计算")
            return {'upFlow': 0, 'downFlow': 0}
        ...
        return {
            'upFlow': round(max(0, up_flow), 2),
            'downFlow': round(max(0, down_flow), 2)
        }
    ...
    def collect(self, package_name: str) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            if uid:
                ...
                if traffic:
                    return self._calculate_flow_rate(package_name, traffic['rx_bytes'], traffic['tx_bytes'])
