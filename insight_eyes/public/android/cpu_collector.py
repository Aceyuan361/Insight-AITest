...
class CPUCollector:
    ...
    def __init__(self, device_id: str, device_profile=None):
        ...
        if self.strategy:
            logger.debug(f"[CPU采集器] 使用策略: {self.strategy.cpu_primary_method}, "
    ...
    def _parse_cpu_from_proc_stat(self, pid: str) -> Optional[float]:
        ...
        try:
            ...
            if pid not in self._last_cpu_data:
                ...
                logger.info(f"[CPU精确算法] 首次采样: pid={pid}, "
            ...
            if delta_app_time < 0:
                logger.warning(f"[CPU精确算法] 进程 CPU 时间异常: delta_app={delta_app_time} "
                             f"(可能是进程重启)")
                # 进程可能重启了，清除缓存重新采样
    ...
    def _get_app_pid(self, package_name: str) -> Optional[str]:
        ...
        try:
            ...
            result = adb.shell(f'ps', self.device_id)
            if result:
    ...
    def _parse_cpu_from_top(self, package_name: str) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            result = adb.shell(f'top -n 1', self.device_id)
                ...
                if '%CPU' in line or 'CPU%' in line:
                    ...
                    for j, part in enumerate(parts):
                        if '%CPU' in part or 'CPU%' in part:
                            cpu_column_index = j
                            logger.info(f"[CPU调试] 找到 CPU 列索引: {cpu_column_index}, 列名: {part}")
                            # 特殊处理：如果表头是 S[%CPU] 格式，数据行会分成两列（S列 + CPU列）
                            # 需要将 CPU 列索引 +1
                            if 'S[%CPU]' in part or '[%CPU]' in part:
                                cpu_column_index += 1
                                logger.info(f"[CPU调试] 检测到 S[%CPU] 格式，CPU 列索引调整为: {cpu_column_index}")
                            break
                    break
                ...
                if package_name in line:
                    ...
                    if cpu_column_index >= 0:
                        if cpu_column_index < len(parts):
                            ...
                            logger.info(f"[CPU调试] 尝试1: 从列 {cpu_column_index} 提取: '{cpu_part}'")

                            result = self._try_extract_cpu(cpu_part, package_name, "按表头列")
                            if result:
                                logger.info(f"[CPU调试] ✓ 尝试1成功: 按表头列{cpu_column_index} -> {result['appCpuRate']}%")
                                return result
                        else:
                            logger.warning(f"[CPU调试] CPU 列索引 {cpu_column_index} 超出数据行范围 (数据行只有 {len(parts)} 列)")
                            logger.info(f"[CPU调试] 列数不匹配: 表头 {column_count} 列 vs 数据行 {len(parts)} 列，尝试智能搜索")
                    ...
                    for idx in range(len(parts) - 2, 0, -1):
                        if '%' in parts[idx]:
                            ...
                            result = self._try_extract_cpu(parts[idx], package_name, f"智能搜索列{idx}")
                            if result:
                                logger.info(f"[CPU调试] ✓ 尝试2成功: 智能搜索列{idx} -> {result['appCpuRate']}%")
                                return result
                    ...
                    for idx in range(len(parts) - 2, 0, -1):
                        ...
                        try:
                            ...
                            if 0 <= value <= 100:
                                logger.info(f"[CPU调试] 尝试2.5: 智能搜索找到数值列 {idx}: '{part}' -> {value}%")
                                result = self._try_extract_cpu(part, package_name, f"智能搜索数值列{idx}")
                                if result:
                                    logger.info(f"[CPU调试] ✓ 尝试2.5成功: 智能搜索数值列{idx} -> {result['appCpuRate']}%")
                                    return result
                    ...
                    if len(parts) >= 10:
                        ...
                        if result:
                            logger.info(f"[CPU调试] ✓ 尝试3成功: 倒数第3列 -> {result['appCpuRate']}%")
                            return result
                        ...
                        if result:
                            logger.info(f"[CPU调试] ✓ 尝试3b成功: 倒数第4列 -> {result['appCpuRate']}%")
                            return result

                    logger.error(f"[CPU调试] ❌ 所有尝试失败，无法从数据行解析 CPU 值: {line[:100]}")
    ...
    def _try_extract_cpu(self, cpu_part: str, package_name: str, method: str) -> Optional[Dict[str, float]]:
        """
        尝试从字符串中提取 CPU 使用率值
        ...
        try:
            ...
            if raw_cpu_value > 0:
                ...
                logger.info(f"[✓] top命令: 原始值={raw_cpu_value}% ÷ {self.cpu_cores}核 = {normalized_cpu:.2f}%")
                return {
                    'appCpuRate': round(normalized_cpu, 2),
            ...
            for line in lines:
                ...
                if package_name in line:
                    ...
                    if match:
                        ...
                        match = re.search(re.escape(package_name) + r':\s*(\d+\.?\d*)%\s+user', line)
                        if match:
                            try:
                                cpu_value = float(match.group(1))
                                app_cpu = max(app_cpu, cpu_value)
                                logger.info(f"[CPU调试] dumpsys提取user部分: {cpu_value}% (行: {line[:100]})")
                            except ValueError:
                                continue
