...
class BatteryCollector:
    ...
    def __init__(self, device_id: str, device_profile=None):
        ...
        if self.strategy:
            logger.debug(f"[电池采集器] 使用策略: {self.strategy.battery_method}, "
        ...
        try:
            ...
            for line in lines:
                ...
                # 解析电流 (单位: mA 或 μA)
                    ...
                    # 尝试多种格式
                    match = re.search(r'current.*?:\s*(-?\d+\.?\d*)\s*(mA|uA|µA)?', line, re.IGNORECASE)
                    if match:
                ...
                # 解析充电状态
                    ...
                    if match:
                        ...
                        elif status_value.lower() in ['charging', 'discharging', 'full', 'not charging']:
    ...
    def _parse_battery_from_health(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 尝试读取电量
            capacity = adb.shell('cat /sys/class/power_supply/battery/capacity 2>/dev/null', self.device_id)
            if capacity:
                battery_data['level'] = int(capacity.strip())

            # 尝试读取电压
            voltage = adb.shell('cat /sys/class/power_supply/battery/voltage_now 2>/dev/null', self.device_id)
            if voltage:
                voltage_uv = int(voltage.strip())
                battery_data['voltage'] = round(voltage_uv / 1000000, 2)  # μV -> V

            # 尝试读取电流
            current = adb.shell('cat /sys/class/power_supply/battery/current_now 2>/dev/null', self.device_id)
            if current:
            ...
            # 尝试读取状态
            status = adb.shell('cat /sys/class/power_supply/battery/status 2>/dev/null', self.device_id)
            if status:
    ...
    def collect(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug(f"[电池采集器] dumpsys 结果: level={result.get('level') if result else None}, temp={result.get('temperature') if result else None}")
            ...
            logger.debug(f"[电池采集器] sysfs 结果: level={result.get('level') if result else None}, temp={result.get('temperature') if result else None}")
            if result and result['level'] > 0:
