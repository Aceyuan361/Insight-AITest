...
class CollectionStrategy:
    ...
    fps_fallback_methods: List[str] = field(default_factory=lambda: [
        "gfxinfo_summary",
...
class DeviceProfileFactory:
    ...
    VENDOR_PATTERNS = {
        ...
        Vendor.HUAWEI: {
            ...
            'build_props': ['ro.build.version.huawei', 'ro.build.version.emui', 'ro.build.version.harmonyos'],
            'product_keywords': ['honor', 'huawei', 'mate', 'p', 'nova', 'yoy']
        },
        Vendor.OPPO: {
            'manufacturer': ['oppo', 'realme', 'oneplus'],
            'build_props': ['ro.build.version.opporom', 'ro.build.version.realme', 'ro.build.version.oneui'],
    ...
    def create(cls, device_id: str) -> DeviceProfile:
        ...
        if len(cls._device_profile_cache) >= cls._max_cache_size:
            ...
            removed = cls._device_profile_cache.pop(oldest_device_id)
            logger.debug(f"[设备缓存] 淘汰最旧的配置: {oldest_device_id}")
        cls._device_profile_cache[device_id] = profile

        logger.info(f"[设备适配] 检测完成: "
                   f"厂商={vendor.value}, ROM={rom_type.value}, "
                   f"Android={android_version}, 型号={model}")
        logger.info(f"[设备适配] 采集策略: FPS={strategy.fps_primary_method}, "
    ...
    def _detect_android_version(cls, device_id: str) -> int:
        ...
        try:
            ...
            if api_level > 0:
                ...
                version_map = {
                    33: 13, 32: 12, 31: 12, 30: 11, 29: 10,
                    28: 9, 27: 8, 26: 8, 25: 7, 24: 7
                }
