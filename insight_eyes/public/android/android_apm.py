...
from insight_eyes.public.android.fps_collector import FPSMonitor
...
from insight_eyes.public.android.device_profile import get_device_profile, DeviceProfile
...
class AndroidAPM:
    ...
    def __init__(self, package_name, device_id, frequency=1.0, surfaceview=True, device_profile=None, **kwargs):
        ...
        if not _validate_package_name(package_name):
            raise ValueError(f"无效的包名格式: {package_name}。包名应遵循 Android 命名规范，如 com.example.app")
        ...
        self.device_profile: DeviceProfile = device_profile if device_profile else get_device_profile(device_id)
        ...
        logger.info(f"[采集策略] FPS={self.strategy.fps_primary_method}, "
        ...
        logger.info(f"初始化 Android APM: package={package_name}, device={device_id}, 采集频率={frequency}秒")
