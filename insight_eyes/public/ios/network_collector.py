...
基于 pymobiledevice3 的 pcapd 服务实现系统级网络流量监控。
...
class NetworkCollector:
    ...
    def collect(self) -> Dict[str, float]:
        ...
        with self._lock:
            ...
            logger.debug(
                f"网络流量: 上传={up_flow_kb:.2f} KB/s, "
                f"下载={down_flow_kb:.2f} KB/s"
            )

            return {
                'upFlow': round(up_flow_kb, 2),
                'downFlow': round(down_flow_kb, 2)
            }
    ...
    def _capture_loop(self):
        """后台捕获循环"""
        try:
            # 创建 lockdown 连接（IOSDeviceAdapter 的 _lockdown_client 未初始化）
            ...
            for packet in pcapd.watch(packets_count=-1):  # 无限捕获
                if not self._running:
                    break
            ...
            logger.error(f"网络捕获线程异常: {type(e).__name__}: {e}")
            # 设置失败标志
            with self._lock:
                self._running = False
