    ...
    def collect(self) -> Dict[str, float]:
        ...
        try:
            ...
            if self._sysmon_service.is_available() or self._sysmon_service.connect():
                logger.debug(f"[MemoryCollector] 调用 get_process_by_bundle_id('{self.bundle_id}')...")
                process = self._sysmon_service.get_process_by_bundle_id(self.bundle_id)

                if process:
                    logger.debug(f"[MemoryCollector] 找到进程，解析内存数据...")
                    ...
                    if self._last_memory_data:
                        logger.info(f"[MemoryCollector] 使用上次采集的值回退: {self._last_memory_data['used_mb']}MB")
                        return self._last_memory_data
                    else:
                        logger.warning(f"[MemoryCollector] 无历史数据可回退")
