...
class SysmonHelper:
    ...
    def _check_availability(self) -> bool:
        ...
        try:
            ...
            logger.info(f"命令: pymobiledevice3 developer dvt sysmon process single")
            ...
            if result.returncode == 0:
                try:
                    ...
                    for i, proc in enumerate(processes[:3]):
                        logger.debug(f"  进程 {i+1}: PID={proc.get('pid')}, name={proc.get('name', 'N/A')}, execName={proc.get('execName', 'N/A')[:50]}")
    ...
    def get_energy_stats(self, pid: int) -> Optional[Dict]:
        ...
        try:
            logger.info(f"===== SysmonHelper: 获取能耗 =====")
