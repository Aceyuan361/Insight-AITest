                        ...
                        if hasattr(res, 'auxiliaries') and res.auxiliaries:
                            ...
                            for item in res.auxiliaries:
                                if isinstance(item, dict):
                                    ...
                                    if (bundle_id in item_name or
                                        bundle_id in item_exec_name):
                                        ...
                                        memory_bytes = phys_footprint if phys_footprint > 0 else mem_resident
                ...
                try:
                    with InstrumentsBase(udid=self.udid) as rpc:
                        ...
                        rpc.process_attributes = [
                            'name', 'pid', 'execName',
                            'cpuUsage', 'physFootprint', 'memResidentSize'
                        ]
                                ...
                                if isinstance(item, dict):
                                    ...
                                    if 'MemUsed' in item or 'PhysMem' in item:
                                        ...
                                        def parse_memory(mem_str):
                                            ...
                                            if isinstance(mem_str, str):
                                                ...
                                                if 'GB' in mem_str or 'GIB' in mem_str:
                                                    return float(mem_str.replace('GB', '').replace('GIB', '').strip()) * 1024
                                                elif 'MB' in mem_str or 'MIB' in mem_str:
                                                    return float(mem_str.replace('MB', '').replace('MIB', '').strip())
                                            return 0.0
                                        ...
                                        if total_mb > 0:
                                            ...
                                            result['percentage'] = round((used_mb / total_mb * 100), 2)
                                            result['found'] = True
