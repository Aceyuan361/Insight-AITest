...
class TimeWindowAccumulator:
    ...
    def __init__(self, window_seconds: int = 120):
        ...
        self._buckets: List[SecondBucket] = [
            SecondBucket(0) for _ in range(window_seconds)
        ]
    ...
    def add(self, cpu: float, memory_mb: float) -> bool:
        ...
        with self._lock:
            ...
            if cpu_valid or mem_valid:
                logger.debug(f"[TimeWindow] 添加数据: second={current_second}, cpu={cpu}(valid={cpu_valid}), memory={memory_mb:.2f}(valid={mem_valid})")
    ...
    def get_aggregated_data(self, frequency_seconds: int) -> Dict[str, float]:
        ...
        with self._lock:
            ...
            # 调试日志
            logger.debug(f"[聚合] 查询时间范围: [{start_second}, {end_second}], 频率={frequency_seconds}秒")
            ...
            for sec in range(start_second, end_second + 1):
                ...
                if bucket.second_timestamp == sec:
                    ...
                    logger.debug(f"[聚合] 第{sec}秒: 时间戳不匹配 (bucket={bucket.second_timestamp}, query={sec})")
                    continue
                ...
                if bucket.has_valid_data():
                    ...
                    if bucket.cpu_values:
                        ...
                        logger.debug(f"[聚合] 第{sec}秒 CPU 平均: {sec_cpu_avg:.2f}% (来自 {cpu_count} 个数据点)")
            ...
            # 调试日志：显示聚合结果
            logger.debug(f"[聚合] 总计: cpu_list长度={len(cpu_list)}, memory_list长度={len(memory_list)}")
            ...
            if cpu_list:
                ...
                if self._last_valid_cpu is not None:
                    ...
                    # 如果从未有过有效数据，返回 0.0
                    final_cpu = 0.0
                    logger.debug(f"[聚合] CPU 从无有效数据，返回 0.0")
            ...
            if memory_list:
                ...
                if self._last_valid_memory is not None:
                    ...
                    # 如果从未有过有效数据，返回 0.0
                    final_memory = 0.0
                    logger.debug(f"[聚合] Memory 从无有效数据，返回 0.0")

            return {
                'cpu_app': round(final_cpu, 2),
                'used_mb': round(final_memory, 2)
            }
    ...
    def on_raw_batch(self, process_list: List[Dict]):
        ...
        with self._cache_lock:
            for proc in process_list:
                ...
                # 调试日志：显示接收到的原始数据
                logger.debug(f"[Throttle] 接收原始数据: pid={pid}, name={proc.get('name')}, cpuUsage={cpu}, physFootprint={phys_footprint}, memory_mb={memory_mb:.2f}")
        ...
        if target_pid is not None:
            with self._cache_lock:
                ...
                if self._target_bundle_id != bundle_id:
                    ...
                    logger.debug(f"[Throttle] 切换目标进程: bundle_id={bundle_id}, pid={target_pid}")
