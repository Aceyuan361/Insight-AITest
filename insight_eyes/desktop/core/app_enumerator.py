...
class BaseAppEnumerator(ABC):
    ...
    def filter_apps(self, apps: List[AppInfo], keyword: str = None,
...
class AndroidAppEnumerator(BaseAppEnumerator):
        ...
        try:
            ...
            for package_name in running_apps:
                if package_name in process_priority:
                    running_apps[package_name]['is_foreground'] = process_priority[package_name]['is_foreground']
                    running_apps[package_name]['oom_adj'] = process_priority[package_name].get('oom_adj', 1000)
            ...
            logger.info(f"应用状态检测完成: 总计 {len(apps)} 个应用，前台: {sum(1 for a in apps if a.status == AppStatus.RUNNING)}，后台: {sum(1 for a in apps if a.status == AppStatus.BACKGROUND)}")
            return apps
    ...
    def _get_process_priorities(self) -> dict:
        ...
        try:
            ...
            for line in output.split('\n'):
                ...
                if current_package and 'adj=' in line:
                    ...
                    if adj_match:
                        ...
                        # - 100 (前台服务)
                        # - 200 (可见进程)
                        is_fg = adj_value in ['_FOREGROUND_APP', '_FOREGROUND', 'FOREGROUND_APP', 'FOREGROUND']
                        if not is_fg:
                            try:
                        ...
                        try:
                            ...
                            process_info[current_package]['oom_adj'] = int(adj_value) if adj_value.isdigit() else 1000
                        except:
                            pass
            ...
            logger.info(f"进程优先级检测完成: 找到 {len(process_info)} 个进程，前台进程: {[p for p, info in process_info.items() if info['is_foreground']]}")
            return process_info
        ...
        try:
            ...
            for line in output.split('\n'):
                ...
                if 'ACTIVITY' in line and '/' in line:
                    ...
                    if len(parts) >= 2:
                        activity_info = parts[1]  # 例如: com.xlive.app/com.xlive.app.activity.MainActivity
    ...
    def get_app_memory_usage(self, package_name: str) -> float:
        ...
        try:
            ...
            output = self._adb.shell(
                f'dumpsys meminfo {package_name}',
                self.device_id,
                timeout=10
            )
            if not output:
    ...
    def get_app_cpu_usage(self, package_name: str) -> float:
        ...
        try:
            ...
            # 使用top命令获取CPU使用率
            output = self._adb.shell(f'top -n 1', self.device_id, timeout=10)
            if not output:
                return 0.0
