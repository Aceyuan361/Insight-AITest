...
export function Dashboard() {
  ...
  if (!stats) return <div style={{ padding: 24, color: 'var(--text-muted)', fontSize: 13 }}>{t('ui.noData')}</div>

  return (
    <div style={{ flex: 1, height: '100%', overflow: 'auto', padding: 16, color: "var(--text-primary)" }}>
      {/* 数字卡片 */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 20 }}>
        <StatCard icon={<TrendingUp size={18} />} label={t('ui.totalRuns')} value={stats.total} color="var(--accent)" />
        <StatCard icon={<CheckCircle size={18} />} label={t('ui.passRate')} value={`${stats.pass_rate}%`} color="var(--success)" sub={`${stats.passed} passed`} />
        <StatCard icon={<XCircle size={18} />} label={t('ui.fail')} value={stats.failed} color="var(--error)" />
        <StatCard icon={<AlertCircle size={18} />} label={t('ui.error')} value={stats.error} color="var(--chart-3)" />
        <StatCard icon={<Clock size={18} />} label={t('ui.avgDuration')} value={`${stats.avg_duration_ms}ms`} color="var(--chart-4)" />
      </div>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: 16, marginBottom: 16 }}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={stats.trend}>
            ...
            <Line type="monotone" dataKey="total" stroke="var(--accent)" strokeWidth={2} name={t('ui.totalRuns')} dot={false} />
            <Line type="monotone" dataKey="passed" stroke="var(--success)" strokeWidth={2} name={t('ui.pass')} dot={false} />
            <Line type="monotone" dataKey="failed" stroke="var(--error)" strokeWidth={2} name={t('ui.fail')} dot={false} />
            <Line type="monotone" dataKey="error" stroke="var(--chart-3)" strokeWidth={1.5} name={t('ui.error')} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: 16 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={stats.top_failures} layout="vertical">
              ...
              />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Bar dataKey="failed" stackId="a" fill="var(--error)" name={t('ui.fail')} />
              <Bar dataKey="error" stackId="a" fill="var(--chart-3)" name={t('ui.error')} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
