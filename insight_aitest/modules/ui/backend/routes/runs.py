# -*- coding: utf-8 -*-
"""执行 / 历史 / 详情 / 统计 / 截图 API（spec F §6）。

路由顺序：静态段（/stats、/{run_id}/screenshot/{idx}）在 /{run_id} 前。
"""
...
from insight_aitest.modules.ui.backend.deps import get_run_db
...
router = APIRouter(prefix="/runs", tags=["ui"])
...
# ===== D 交互（可被 monkeypatch 替换，同 E） =====
#
# 注意：F 与 D 同进程，不能 HTTP 自调用（死锁），直接复用 D 的 TestCaseDatabase。
...
def _fetch_case_from_d(case_id: int) -> dict | None:
    """从 D 读 UI 用例（含 content）。不存在返回 None。"""
...
def _make_agent_factory() -> Callable:
    """生产 agent 工厂。测试 monkeypatch 替换为返回 FakeAgent 的工厂。

    先校验视觉模型配置（API key / 模型名非空），不通过则抛 VisionConfigError，
    让调用方（execute_case 端点）能 catch 并返回 400 友好提示。
    """
    from insight_aitest.modules.ui.backend.engine.executor import (
        _check_llm_config,
...
async def execute_case(
    ...
    db: UIRunDatabase = Depends(get_run_db),
) -> dict:
    ...
    try:
        ...
        raise HTTPException(422, f"用例 content 非合法 UI schema: {e}")
    ...
    try:
        run = await execute(
            ...
            browser_config=body.browser_config,
        )
...
@router.get("/stats")
async def stats(case_id: int | None = Query(None), db: UIRunDatabase = Depends(get_run_db)) -> dict:
...
@router.get("/{run_id}")
async def get_run(run_id: int, db: UIRunDatabase = Depends(get_run_db)) -> dict:
...
async def get_screenshot(run_id: int, step_index: int, db: UIRunDatabase = Depends(get_run_db)):
    run = db.get_run(run_id)
    if not run:
        raise HTTPException(404, "执行记录不存在")
    if step_index < 0 or step_index >= len(run.steps):
        raise HTTPException(404, "步骤不存在")
    path = run.steps[step_index].screenshot
    if not path or not os.path.exists(path):
        raise HTTPException(404, "该步无截图")
    return FileResponse(path)


@router.delete("/{run_id}")
async def delete_run(run_id: int, db: UIRunDatabase = Depends(get_run_db)) -> dict:
